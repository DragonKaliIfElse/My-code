...
def salvar_relatorio():
    ...
    for i, categoria in enumerate(categorias_preenchidas):
        ...
        incluir = ''
        for linha in linhas_categoria:
            if linha["entries"]["categoria"] == categoria: 
                if linha["entries"]["categoria"] == True:
                    incluir = 'Sim'
                else:
                    incluir = 'Não'
                celula = xe.encontrar_celula_editavel(ws,line+indice,1)
                celula.value = linha["entries"]["item"].get()
                celula = xe.encontrar_celula_editavel(ws,line+indice,2)
                celula.value = linha["entries"]["tipo"].get()
                celula = xe.encontrar_celula_editavel(ws,line+indice,3)
                celula.value = linha["entries"]["descricao"].get()  
                celula = xe.encontrar_celula_editavel(ws,line+indice,5)
                celula.value =  linha["entries"]["qtde"].get()
                celula = xe.encontrar_celula_editavel(ws,line+indice,6)
                celula.value = linha["entries"]["diaria"].get() 
                celula = xe.encontrar_celula_editavel(ws,line+indice,7)
                celula.value = linha["entries"]["subtotal"].get() 
                celula = xe.encontrar_celula_editavel(ws,line+indice,8)
                celula.value = linha["entries"]["total"].get() 
                celula = xe.encontrar_celula_editavel(ws,line+indice,9)
    ...
    ws.merge_cells(start_row=ult, start_column=1, end_row=ult, end_column=9)
    wb.save(arquivo_editavel)
