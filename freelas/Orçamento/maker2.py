...
class DatabaseViewer:
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            try:
                ...
                print("Colunas originais:", list(self.df_original.columns))
                ...
                print("Colunas após renomeação:", list(self.df.columns))
    ...
    def gerar_relatorio(self):
        """Gera relatório a partir dos dados da base de dados - CORRIGIDO: usa Orçamento Total"""
        try:
            if self.df.empty:
                messagebox.showwarning("Aviso", "Não há dados para gerar o relatório.")
                return
            
            # Verificar se há dados necessários - USANDO NOVOS NOMES
            colunas_necessarias = ['Evento', 'Categoria', 'Orçamento Total']
            for coluna in colunas_necessarias:
            ...
            # Calcular totais por categoria - USANDO NOVO NOME
            try:
                # Converter coluna Orçamento Total para numérico
                self.df['Orçamento Total'] = pd.to_numeric(self.df['Orçamento Total'], errors='coerce').fillna(0)
                totais_categoria = self.df.groupby('Categoria')['Orçamento Total'].sum()
            except Exception as e:
    ...
    def _gerar_relatorio_excel(self, dados_orcamento, dados_centro_custo, df_itens):
        """Gera o relatório Excel com os dados fornecidos - CORRIGIDO: usa cópia e preserva formatação"""
        try:
            # Criar cópia do arquivo para edição
            ...
            for i, categoria in enumerate(categorias_para_processar):
                ...
                # Preservar formatação original - usar funções do xe
                try:
                ...
                for _, item in itens_categoria.iterrows():
                    # Usar xe.encontrar_celula_editavel para preservar formatação
            ...
            # Aplicar totais finais apenas se pelo menos uma categoria foi processada
            if categorias_processadas > 0:
                wb = load_workbook(arquivo_editavel)
                ws = wb["ENVIO P CLIENTE"]
                df = pd.read_excel(arquivo_editavel, sheet_name="ENVIO P CLIENTE")
...
# A classe BudgetApp permanece a mesma, mas vou corrigir o método salvar_relatorio para usar a cópia
class BudgetApp:
    ...
    # ... (outros métodos da classe BudgetApp permanecem iguais)
    ...
    def salvar_relatorio(self) -> None:
        """Salva o relatório completo - CORRIGIDO: usa cópia do arquivo"""
        try:
            # Criar cópia do arquivo para edição
            arquivo_editavel = xe.cop_sheet(str(self.arquivo))
            wb = load_workbook(arquivo_editavel)
            ...
            # Garantir que todas as categorias tenham pelo menos 1 item
            for cat in nomes_categorias:
